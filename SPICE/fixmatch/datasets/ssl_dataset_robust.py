...
#Adding FashionIQ data Mean
mean['fashionIQ'] = [0.485, 0.456, 0.406]

std['cifar10'] = [x / 255 for x in [63.0, 62.1, 66.7]]
std['cifar100'] = [x / 255 for x in [68.2,  65.4,  70.4]]
std['stl10'] = [0.229, 0.224, 0.225]
std['npy'] = [0.229, 0.224, 0.225]
#Adding FashionIQ data std
std['fashionIQ'] = [0.229, 0.224, 0.225]


def get_transform(mean, std, dataset, train=True):
    if dataset in ['cifar10', 'cifar20', 'cifar100']:
        crop_size = 32
    elif dataset in ['stl10', 'npy','fashionIQ']: #Added fashioniq
        crop_size = 96
    else:
        raise TypeError
    if train:
...
class SSL_Dataset:
    ...
    def get_data(self):
        ...
        elif self.name == "fashionIQ":
            #Implement FASHIONIQ fixmatch dataset?
            dset = FASHIONIQ(root=self.data_dir)
